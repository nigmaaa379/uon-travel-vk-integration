...
function sec(title,v){return v&&(!Array.isArray(v)||v.length)?'<div class="c-sec"><h4>'+esc(title)+'</h4>'+block(v)+'</div>':''}
function build(list){
...
html+=sec('Таможня и вывоз валюты',c.customs)
...
if(c.source)html+='<a class="c-src" href="'+esc(c.source)+'" target="_blank" rel="nofollow noopener">Источник: Русский Экспресс</a>'
body.innerHTML=html
...
function parsed(){
return fetch('/data/visa-rules.json',{cache:'no-store'}).then(function(r){return r.ok?r.json():{}}).catch(function(){return {}})
}
Promise.all([Promise.all(FILES.map(load)),parsed()]).then(function(res){
var remote=res[1]||{}
var seen={},list=[]
...
seen[c.id]=1
var r=remote[c.id]
if(r)['passport','insurance','currency','visaRules','children','alert','source'].forEach(function(k){if(r[k]&&(!Array.isArray(r[k])||r[k].length))c[k]=r[k]})
...
if(!c.children)c.children=['Ребёнок выезжает из России хотя бы с одним из родителей, усыновителей, опекунов или попечителей (ФЗ №114-ФЗ, ст. 20–21).','При поездке с одним родителем согласие второго не требуется, если от него не поступало заявления о несогласии на выезд ребёнка.','Без сопровождения родителей ребёнку нужно нотариально оформленное согласие родителей с указанием срока выезда и стран поездки.','У каждого ребёнка должен быть свой загранпаспорт. Если фамилии ребёнка и сопровождающего родителя разные, возьмите документы, подтверждающие родство.']
if(!c.customs)c.customs='С 02.03.2022 временно запрещён вывоз из Российской Федерации наличной иностранной валюты в сумме, превышающей эквивалент 10 000 USD.'
list.push(c)
})
