...
test('the traffic source reaches the U-ON card and the journal', async()=>{const{SiteComplianceService}=await import('../src/site-compliance.js');const store=await freshStore();const leads=[];
const svc=new SiteComplianceService({siteRoot:new URL('../web',import.meta.url).pathname,store,uon:{createQualifiedLead:async t=>{leads.push(t);return{id:3001}}},notifier:{notify:async()=>({ok:true})},evidenceSecret:'s'.repeat(32),logger:{error(){},warn(){}}});
...
const base={name:'Анна Смирнова',phone:'+79001234567',personalConsent:true,countryInfoAcknowledged:true};
await send({...base,source:{utm_source:'yandex',utm_medium:'cpc',utm_campaign:'turkey',referrer:'https://yandex.ru/',landing:'/'}});
await send({...base,source:{referrer:'https://vk.com/turagentgorevalubov',landing:'/countries/turkey.html'}});
await send({...base});
srv.close();
assert.match(leads[0].wishes,/utm_source=yandex/,'Метка кампании не попала в карточку U-ON');
assert.match(leads[0].wishes,/utm_campaign=turkey/);
assert.match(leads[1].wishes,/vk\.com/,'Переход из ВК не виден в карточке');
assert.ok(!/Источник/.test(leads[2].wishes),'При прямом заходе лишней строки быть не должно');
const sources=store.listConsentEvidence().map(e=>e.source);
assert.ok(sources.some(s=>s.includes('utm_source=yandex')),'Источник не сохранён в журнале согласий');
assert.ok(sources.includes('прямой заход'),'Прямой заход должен помечаться явно')});
