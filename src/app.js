...
async function serveSite(req, res) {
  ...
  const safePath = normalize(requested).replace(/^(\.\.(\/|\\|$))+/, ''); const filePath = join(siteRoot, safePath); if (!filePath.startsWith(siteRoot)) return false;
  try {
    if (!(await stat(filePath)).isFile()) return false;
    res.writeHead(200, { 'content-type': mime[extname(filePath).toLowerCase()] || 'application/octet-stream', 'x-content-type-options': 'nosniff', 'referrer-policy': 'strict-origin-when-cross-origin', 'x-frame-options': 'SAMEORIGIN', 'permissions-policy': 'camera=(), microphone=(), geolocation=()', 'content-security-policy': "default-src 'self'; img-src 'self' data: https://images.unsplash.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com; script-src 'self' 'unsafe-inline'; connect-src 'self'; frame-src https://yandex.ru https://yandex.com; object-src 'none'; base-uri 'self'; form-action 'self'", 'cache-control': extname(filePath) === '.html' ? 'no-cache' : 'public, max-age=86400' });
    if (req.method === 'HEAD') return res.end(), true; createReadStream(filePath).pipe(res); return true;
  } catch { return false; }
}
function validateSiteLead(body) {
  ...
  if (body.email && (typeof body.email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email.trim()))) return { error: 'Проверьте email.' };
  for (const [key, max] of Object.entries({ name: 120, phone: 32, email: 254, destination: 160, group: 160, wishes: 2000 })) if (typeof body[key] === 'string' && body[key].length > max) return { error: `Поле ${key} слишком длинное.` };
  return null;
}
export function buildServer({ config, store, vk, uon, notifier, botCore, platforms, logger = console }) {
  const siteLeadAttempts = new Map();
  const allowSiteLead = (req) => { const forwarded = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim(); const key = forwarded || req.socket.remoteAddress || 'unknown'; const now = Date.now(); const recent = (siteLeadAttempts.get(key) || []).filter(t => now - t < 600_000); if (recent.length >= 5) return false; recent.push(now); siteLeadAttempts.set(key, recent); return true; };
  const processBot = async (platform, parsed) => {
    if (!parsed || !(await store.claimEvent(parsed.eventId))) return;
    try { try { await platforms[platform].ack?.(parsed.ackId); } catch (error) { logger.warn('Callback acknowledgement failed', { platform, error: error.message }); } const output = await botCore.handle(platform, parsed.userId, parsed.input); await platforms[platform].send(parsed.userId, output); await store.markProcessed(parsed.eventId); }
  ...
  return createServer(async (req, res) => {
    ...
    try {
      if (req.method === 'POST' && req.url === '/api/site/leads') {
        if (!String(req.headers['content-type'] || '').toLowerCase().startsWith('application/json')) return json(res, 415, { error: 'Content-Type должен быть application/json.' });
        const body = await readJson(req); const invalid = validateSiteLead(body); if (invalid?.spam) return json(res, 200, { ok: true, leadId: 'accepted' }); if (invalid) return json(res, 422, invalid);
        if (!allowSiteLead(req)) { res.setHeader('retry-after', '600'); return json(res, 429, { error: 'Слишком много заявок. Попробуйте через 10 минут.' }); }
      ...
      if (req.method === 'POST' && req.url === '/vk/callback') {
        const event = await readJson(req); if (String(event.group_id) !== String(config.vk.groupId) || event.secret !== config.vk.secret) return json(res, 403, { error: 'Forbidden' }); if (event.type === 'confirmation') { res.writeHead(200, { 'content-type': 'text/plain' }); return res.end(config.vk.confirmationCode); } if (event.type !== 'message_new') { res.writeHead(200); return res.end('ok'); }
        const eventId = event.event_id || `${event.object?.message?.id}:${event.object?.message?.peer_id}`; res.writeHead(200); res.end('ok'); const message = event.object?.message; if (!message?.from_id || !message?.text || !(await store.claimEvent(eventId))) return;
